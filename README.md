...
- **Na frente do card:** os **dias úteis desde a criação do card** (ex.: `🗓 6d`
  em fila, `▶ 6d` em andamento). Aparece **automaticamente desde que o card
  existe** — não depende de clicar em "Iniciar" (que marca só o trabalho). Fins
  de semana não contam; congela ao ir para "Concluído". A data de criação vem do
  próprio ID do card (client-side). Cards em fila também aparecem no relatório.
